...
optimizer = AdamW(net.parameters().astorch(), lr=1e-4, weight_decay=1e-5)
...
for epoch in range(2001):
    ...
    if epoch % 50 == 0:
        birdcalledge.training.save_checkpoint(
            results_dir / 'checkpoints' / f"synnetqatv2_pretraining_replicate_checkpoint_epoch_{epoch:04d}.pt",
            net,
            optimizer,
            epoch,
            this_loss,
...
np.save(results_dir / 'synnetqatv2_pretraining_replicate_loss.npy', loss_t) 
